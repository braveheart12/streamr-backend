...
public class CSVImporter implements Iterable<LineValues> {
	...
	public class Schema {
		...
		public final CSVParser[] parsersToTry = { new CSVParser(','),
				new CSVParser('\t'), new CSVParser(';') };

		// TODO: expand, support ISO standards
		public final SimpleDateFormat[] dateFormatsToTry = { new SimpleDateFormat(
		...
		private void detect(InputStream is) throws IOException {
			...
			try {
				...
				while (undetectedSchemaEntries > 0
						&& (line = reader.readLine()) != null) {

					// Try to detect separator format and parse headers from
					// first line
					if (lineCount == 0) {
					...
					// On subsequent lines, try to detect the format of each
					// individual column
					else {
						...
						if (fields.length < entries.length) {
							throw new RuntimeException(
									"Unexpected number of columns on row "
											+ lineCount);
						}
				...
				if (undetectedSchemaEntries > 0) {
					StringBuilder sb = new StringBuilder(
							"Format could not be detected for the following columns: ");
		...
		private String[] detectHeaders(String line) throws IOException {
			...
			while ((headers == null || headers.length < 2)
					&& parserIdx < parsersToTry.length) {
			...
			if (headers.length < 2) {
				throw new RuntimeException(
						"Sorry, couldn't determine format of csv file!");
			} else
				return headers;
		}
		...
		private SchemaEntry detectEntry(String value, String name) {
			...
			// Else treat it as string
				...
				return new SchemaEntry(name, "string");
		}
		...
		LineValues parseLine(String line) throws IOException, ParseException {
			...
			if (values.length < entries.length) {
				throw new RuntimeException(
						"Unexpected number of columns on line: " + line);
			}
